...
const BarChart = ({ data, columns }) => {
    ...
    const findByKeyword = (indices) => {
        return indices.find(idx =>
            labelKeywords.some(kw => columns[idx].toLowerCase().includes(kw))
        );
    };

    labelIndex = findByKeyword(nonNumericIndices);
    if (labelIndex === undefined || labelIndex === -1) labelIndex = nonNumericIndices[0];
    if (labelIndex === undefined || labelIndex === -1) labelIndex = findByKeyword(numericIndices.filter(i => i !== chartIndex));
    if (labelIndex === undefined || labelIndex === -1) labelIndex = 0;

    const maxVal = Math.max(...data.map(r => Number(r[chartIndex]) || 0), 1);
...
const SqlTerminal = () => {
    ...
    const copySelected = (format) => {
        if (!results || selectedRows.size === 0) return;

        const selectedData = results.rows.filter((_, i) => selectedRows.has(i));
        let content = '';
        ...
        if (format === 'json') {
            ...
            const headers = results.columns.join('\t');
            const rows = selectedData.map(row => row.join('\t')).join('\n');
            content = `${headers}\n${rows}`;
        }
                            ...
                            {filteredSaved.length === 0 ? (
                                ...
                                filteredSaved.map((q) => (
                                    ...
                                    >
                                        ...
                                        <button
                                            onClick={(e) => deleteSavedQuery(e, q.id)}
                                        ...
                                        <tbody className="divide-y divide-[#2e2e2e]/50">
                                            {results.rows?.map((row, i) => (
                                                <tr
                                                    ...
                                                    onClick={() => toggleRow(i)}
                                                >
                                                    <td className="w-10 px-6 py-3 border-r border-[#2e2e2e]/30" onClick={(e) => e.stopPropagation()}>
                                                        <input
                                                            type="checkbox"
                                                            className="w-3.5 h-3.5 rounded border-zinc-700 bg-zinc-900 checked:bg-primary checked:border-primary focus:ring-0 transition-all cursor-pointer accent-primary"
