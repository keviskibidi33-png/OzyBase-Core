...
const SchemaVisualizer = ({ viewMode = 'user' }: any) => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            data.tables.forEach((table: any, i: any) => {
                const col = i % cols;
        ...
        const handleKeyDown = (e: any) => {
            ...
            if (e.ctrlKey || e.metaKey) {
                if (['Equal', 'NumpadAdd', 'Plus'].includes(e.code) || (e.key === '+' || e.key === '=')) {
                    e.preventDefault();
                    setScale((prev: any) => Math.min(prev + 0.1, 2));
                } else if (['Minus', 'NumpadSubtract', 'Hyphen'].includes(e.code) || (e.key === '-')) {
                    e.preventDefault();
                    setScale((prev: any) => Math.max(prev - 0.1, 0.2));
    ...
    useEffect(() => {
        ...
        const handleWheel = (e: any) => {
            if (e.ctrlKey) {
                ...
                setScale((prev: any) => Math.min(Math.max(prev + delta, 0.2), 2));
            }
        };
    ...
    const handleNodeMouseDown = (e: any, tableName: any) => {
    ...
    const handleExportCSV = () => {
        ...
        tablesToExport.forEach((table: any) => {
        ...
        <div className="flex flex-col h-full bg-[#0c0c0c] overflow-hidden text-zinc-300 font-sans relative">
            ...
            <div className="absolute top-4 left-4 z-50 flex items-center gap-4 bg-[#1a1a1a]/80 backdrop-blur-md border border-[#2e2e2e] p-2 px-4 rounded-xl shadow-2xl">
                ...
                <div className="flex items-center gap-1">
                    <button onClick={() => setScale((s: any) => Math.max(s - 0.1, 0.2))} className="p-1.5 hover:bg-zinc-800 rounded text-zinc-400 hover:text-white"><ZoomOut size={16} /></button>
                    <span className="text-[10px] font-mono w-10 text-center">{Math.round(scale * 100)}%</span>
                    <button onClick={() => setScale((s: any) => Math.min(s + 0.1, 2))} className="p-1.5 hover:bg-zinc-800 rounded text-zinc-400 hover:text-white"><ZoomIn size={16} /></button>
                </div>
                <div className="w-[1px] h-4 bg-zinc-800" />
            ...
            >
                ...
                >
                    <svg className="absolute top-0 left-0 w-[5000px] h-[5000px] pointer-events-none z-0">
                        {(schema.relationships || []).filter((rel: any) =>
                            filteredTables.find((t: any) => t.name === rel.from_table) &&
                            filteredTables.find((t: any) => t.name === rel.to_table)
                        ).map((rel: any, i: any) => {
                    ...
                    {filteredTables.map((table: any) => {
                        ...
                        return (
                            <div
                                key={table.name}
                                onMouseDown={(e: any) => handleNodeMouseDown(e, table.name)}
